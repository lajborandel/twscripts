...
// @namespace    @lajbor
// @version      2.7-ui // ZMĚNA: Verze zvýšena
// @description  Původní funkční autostavění + panel. Priorita dolů; při hlášce o kapacitě postaví Sklad/Selský dvůr a pokračuje v dolech. Přidány cíle.
...
  const BUILDINGS = [
    { key: 'wood',     label: 'Dřevo',          defaultTarget: 30 }, // Nastaveno na 30
    { key: 'stone',    label: 'Hlína',          defaultTarget: 30 }, // Nastaveno na 30
    { key: 'iron',     label: 'Železo',         defaultTarget: 30 }, // Nastaveno na 30
    { key: 'storage',  label: 'Sklad',          defaultTarget: 30 }, // Nastaveno na 30
    { key: 'farm',     label: 'Selský dvůr',    defaultTarget: 30 }, // Nastaveno na 30
    { key: 'main',     label: 'Hlavní budova',  defaultTarget: 25 }, // Nastaveno na 25
    { key: 'barracks', label: 'Kasárna',        defaultTarget: 25 }, // Nastaveno na 25
    { key: 'smith',    label: 'Kovárna',        defaultTarget: 20 }, // Nastaveno na 20
    { key: 'stable',   label: 'Stáj',           defaultTarget: 20 }, // Nastaveno na 20
    { key: 'garage',   label: 'Dílna',           defaultTarget: 15 }, // Nastaveno na 15
    { key: 'market',   label: 'Trh',            defaultTarget: 20 }, // Nastaveno na 20
    { key: 'wall',     label: 'Hradba',         defaultTarget: 20 }, // Nastaveno na 20
    { key: 'place',    label: 'Shromaždiště',   defaultTarget: 1  }, // Panský dvůr je 'place' (Shromaždiště) level 1. Měl jsi na mysli 'Panský dvůr' (statue) nebo 'Shromaždiště' (place)? Pokud Panský dvůr, použij 'statue' s cílem 1.
    { key: 'statue',   label: 'Socha',          defaultTarget: 1  }, // Nastaveno na 1
    { key: 'hide',     label: 'Skrýš',          defaultTarget: 0  } // Nastaveno na 0
  ];
...
      return { current: Math.max(0, next-1), next };
    }
...
    const host = document.querySelector('#buildqueue') || document.querySelector('#content_value') || document.body;
...
        <td style="text-align:right;padding:6px;font-family:monospace">${current} ➜ ${next}</td>`;
...
    if (!buttons.length) { log("⚠️ Vše zakázané."); return false; }
...
    // Tohle ignoruje UI cíle a „enabled“ – jde o jednorázové odblokování dolů
...
    const mainScreen = document.querySelector("#buildqueue") || document.querySelector(".btn-build");
    if (mainScreen) {
      observer.disconnect();
      renderControlPanel();
      log("✅ Stránka hlavní budovy načtena.");
      startLoop();
    }
  });
